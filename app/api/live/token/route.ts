...
export async function POST(request: Request) {
  try {
    ...
    const rawIdentity = body.identity || `viewer-${Math.random().toString(36).substring(2, 9)}`;

    const sanitizedRoom = String(rawRoom).replace(/[^a-zA-Z0-9_-]/g, '').substring(0, 64) || 'efootball-live-test';
    const sanitizedIdentity = String(rawIdentity).replace(/[^a-zA-Z0-9_-]/g, '').substring(0, 64) || `viewer-${Math.random().toString(36).substring(2, 9)}`;
...
export async function GET(request: Request) {
  try {
    ...
    const rawIdentity = searchParams.get('identity') || `viewer-${Math.random().toString(36).substring(2, 9)}`;
